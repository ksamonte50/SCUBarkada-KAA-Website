...
* Description: This program implements a Stop and Wait reliable protocol on top of UDP to provide a reliable transport service while considering loss
*/
...
Packet serverReceive(int sockfd, struct sockaddr *address, socklen_t *addrlen, int seqnum) {
    ...
    while (1) {
	    ...
	    recvfrom(sockfd, &packet, sizeof(packet), 0, (struct sockaddr *)&clienAddr, addrlen);	     
